...
const CaseSummary = (props) => {
    ...
    const [emergencyforasaps, setemergencyforasaps] = useState(null)
    ...
    const handleChangeEmergency = (value) => {
        if (value.value === "Yes") {
            setemergencyforasaps(true)
        } else {
            setemergencyforasaps(false)
        }
    ...
    const callToCaseSummary = () => {
        ...
        if (asaps.length) {
            ...
            objforasa_ps_data.id = asaps.id
            objforasa_ps_data.emergency = emergencyforasaps
            asa_ps_data.push(objforasa_ps_data)
        }
        let surgery_types_data = []
        if (surgerySubType.length) {
        ...
        //console.log(age,ageType,fileName,gender,height,heightType,weightType,weight,patientType,timeOfLastFoodOrDrink,ingestedMaterial.label,asa_ps_data,surgery_types_data,surgery_histories_data,anesthesia_types_data,medical_histories_data,medications_data,anesthesia_histories_data,allergies_data,custom_allergies_data,custom_medications_data,custom_surgery_types_data)
        var token = localStorage.getItem("token")
        fetch(API_ROOT + `save-case-summary?token=${token}`, {
            ...
            body: JSON.stringify({
                ...
                "npo_status_materials": ingestedMaterial.label,
    ...
    return (
        <div  >
            ...
            <SaveCaseModal onClickSave={() => nameModalVisible()} modalIsOpen={modalIsOpen} onClose={() => closeModal()} />
            <SaveCaseNameModal onCaseNameChange={(value) => changeCaseName(value)} closeNameModal={() => closeNameModal()} postRequest={() => callToCaseSummary()} saveVisible={nameVisble} onClose={() => closeModal()} />
            <Header onMenuClick={() => burgerMenuClick()} />
            <div onClick={() => nameModalVisible()} className="casename-container" >
            ...
            <div className="all-action-container" >
                <div className="tab-container" >
                    ...
                    <Link className="tab-container-tabs" to="/actionsummary"   >ACTION SUMMARY</Link>
                ...
                {questinNo == 0 ?
                    <div className="result-container" >
                        ...
                        <div className="next-button-container" >
                            ...
                            <div onClick={() => nameModalVisible()} /*onClick={() => }*/ className="next-button" >
                                NEXT
                            </div>
                        </div>
                ...
                {questinNo == 1 ?
                    <div className="question-container" >
                        ...
                        <div className="age-input-container" >
                            <input value={age} onChange={(e) => setAge(e.target.value)} placeholder="ANSWER" className="age-input" />
                            ...
                            {optionsforage.map((data) => {
                                if (data.value === ageType) {

                                    return (
                                        <div className="age-selected-type-button" onClick={() => handleChangeAgeTpe(data)} >{data.label}</div>

                                    )
                                } else {

                                    return (
                                        <div className="age-type-button" onClick={() => handleChangeAgeTpe(data)} >{data.label}</div>
                        ...
                        <div className="gender-input-container" >
                            ...
                            {optionsforgender.map((data) => {
                                if (data.value === gender) {
                                    return (
                                        <div className="gender-selected-type-button" onClick={() => handleChangeGender(data)} >{data.label}</div>
                                    )
                                } else {
                                    return (
                                        <div className="gender-type-button" onClick={() => handleChangeGender(data)} >{data.label}</div>
                                    )
                                }
                            })}
                        ...
                        <div className="patientype-input-container" >
                            ...
                            <div className="patient-type-button-container" >
                                ...
                                {patientTypeArray.map((data) => {
                                    if (data.id === patientType) {
                                        return (
                                            <div className="patient-selected-type-button" onClick={() => handleChangePatientType(data)} >{data.label}</div>
                                        )
                                    } else {
                                        return (
                                            <div className="patient-type-button" onClick={() => handleChangePatientType(data)} >{data.label}</div>
                ...
                {questinNo == 4 ?
                    <div className="question-container" >
                        ...
                        <div className="height-input-container" >
                            <input value={height} onChange={(e) => setHeight(e.target.value)} placeholder="ANSWER" className="height-input" />
                            ...
                            {optionsForHeight.map((data) => {
                                if (data.value === heightType) {
                                    return (
                                        <div className="height-selected-type-button" onClick={() => handleChangeHeightType(data)} >{data.label}</div>
                                    )
                                } else {
                                    return (
                                        <div className="height-type-button" onClick={() => handleChangeHeightType(data)} >{data.label}</div>
                ...
                {questinNo == 5 ?
                    <div className="question-container" >
                        ...
                        <div className="weight-input-container" >
                            <input value={weight} onChange={(e) => setWeight(e.target.value)} placeholder="ANSWER" className="weight-input" />
                            ...
                            {optionsForWeight.map((data) => {
                                if (data.value === weightType) {
                                    return (
                                        <div className="weight-selected-type-button" onClick={() => handleChangeWeightType(data)} >{data.label}</div>
                                    )
                                } else {
                                    return (
                                        <div className="weight-type-button" onClick={() => handleChangeWeightType(data)} >{data.label}</div>
                        ...
                        <div className="ASA-PS-input-container" >
                            <div className="asa-ps-button-container" >
                                ...
                                {AsapaArray.map((data) => {
                                    if (data === asaps) {
                                        return (
                                            <div className="asaps-selected-type-button" onClick={() => handleChangesub_asapa(data)} >{data.label}</div>
                                        )
                                    } else {
                                        return (
                                            <div className="asa-ps-type-button" onClick={() => handleChangesub_asapa(data)} >{data.label}</div>
                            ...
                            {optionsforEmergency.map((data) => {
                                if (data.value === emergency) {
                                    return (
                                        <div className="emergency-selected-type-button" onClick={() => handleChangeEmergency(data)} >{data.label}</div>
                                    )
                                } else {
                                    return (
                                        <div className="weight-type-button" onClick={() => handleChangeEmergency(data)} >{data.label}</div>
                ...
                {questinNo == 7 ?
                    <div className="question-container" >
                        ...
                        <div className="Surgery-type-input-container" >
                            <Select value={surgeryType} onChange={(value) => handleChangeSurgeryType(value)} placeholder="SURGERY-TYPE" className="Surgery-type-dropdown" id="patien-type" options={surgeryTypeArray} />
                        </div>
                        {sub_surgeryArray.length !== 0 ?
                            <div className="Surgery-type-input-container" >
                                <Select value={surgerySubType} onChange={(value) => handleChangesub_surgeryType(value)} placeholder="SURGERY-TYPE" className="Surgery-type-dropdown" id="patien-type" options={sub_surgeryArray} />

                            </div> : <></>
                        }
                        <div className="medication-extra-type-container" >
                            <input className="medication-input" onChange={(e) => setcategoryValue(e.target.value)} placeholder="enter not listed type" />
                            <div className="add-more-button" onClick={() => addotherIntoSurgeryTypeCategory(categoryValue)} >Add</div>
                        </div>
                        {
                            otherSurgeryTypeArray.map((data) => {
                                return (
                ...
                {questinNo == 8 ?
                    <div className="question-container" >
                        ...
                        <div className="ANESTHESIA-type-input-container" >
                            <Select isMulti={true} value={anesthesia_type} onChange={(value) => handleChangeAnesethesiaType(value)} placeholder="ANESTHESIA TYPE" className="ANESTHESIA-type-dropdown" id="patien-type" options={anesthesia_type_list} />
                        ...
                        <div className="Surgery-type-input-container" >
                            {
                                subanesthesia_type.length !== 0 ?
                                    <Select isMulti={true} onChange={(value) => handleChangesub_anesthesia_type(value)} placeholder="SURGERY-TYPE" className="Surgery-type-dropdown" id="patien-type" options={subanesthesia_type} />
...

                                                                    {
                                                                        medicalHistoryArray.map((selecteddata) => {
                                                                            if (selecteddata.name === data1.name) {
                                                                                return (
                                                                                    <div><div className="option-box-container" >
                                                                                        <input checked={selecteddata.name === data1.name ? true : false} onChange={(e) => onSiteChanged(e, data1)} type="radio" value="Yes" name={data1.name} /> Yes
                                                                                        <input onChange={(e) => onSiteChanged(e, data1)} type="radio" value="No" name={data1.name} /> No
                                                                                    </div>
                                                                                    </div>
                                                                                )
                                                                        ...
                                                                        <div className="option-box-container" >
                                                                            <input onChange={(e) => onSiteChanged(e, data1)} type="radio" value="Yes" name={data1.name} /> Yes
                                                                            <input onChange={(e) => onSiteChanged(e, data1)} type="radio" value="No" name={data1.name} /> No
                                                                        </div> : <></>
                                                                    }
                                                                </div>
                ...
                {questinNo == 10 ?
                    <div className="question-container" >
                        ...
                        <div className="medications-input-container" >
                                ...
                                medicationsArray.map((data1) => {
                                    return (
                                        <div className="sub-question-container" >
                                            <div>{data1.label}</div>
                                            {
                                                medication.map((selecteddata) => {
                                                    if (selecteddata.drug_name === data1.drug_name) {
                                                        return (
                                                            <div><div className="option-box-container" >
                                                                <input checked={selecteddata.drug_name === data1.drug_name ? true : false} onChange={(e) => onSiteChanged(e, data1)} type="radio" value="Yes" name={data1.drug_name} /> Yes
                                                                <input onChange={(e) => onSiteChanged(e, data1)} type="radio" value="No" name={data1.drug_name} /> No
                                                            </div>
                                                            </div>
                                                        )
                                            ...
                                            {medication.length === 0 ?
                                                <div className="option-box-container" >
                                                    <input onChange={(e) => onMedicationValue(e, data1)} type="radio" value="Yes" name={data1.drug_name} /> Yes
                                                    <input onChange={(e) => onMedicationValue(e, data1)} type="radio" value="No" name={data1.drug_name} /> No
                                                </div> : <></>
                                            }
                                        </div>
                            ...
                            <div className="medication-extra-type-container" >
                                <input className="medication-input" onChange={(e) => setcategoryValue(e.target.value)} placeholder="enter not listed type" />
                                <div className="add-more-button" onClick={() => addotherIntoMedicationCategory(categoryValue)} >Add</div>
                            </div>
                            {otherMedicationArray.map((data) => {
                                return (
                                    <div>{data}</div>
                ...
                {questinNo == 11 ?
                    <div className="question-container" >
                        ...
                        <div className="Surgery-type-header-container" >
                            QUESTION {questinNo} OF 14- SURGERY HISTORY?
                        </div>
                        <div className="Surgery-type-input-container" >
                            <Select value={surgeryHistory} onChange={(value) => handleChangeSurgeryHistory(value)} placeholder="SURGERY-HISTORY" className="Surgery-type-dropdown" id="patien-type" options={surgeryHistoryArray} />

                        </div>
                        <div className="Surgery-type-input-container" >
                            {
                                sub_surgeryHistoryArray.length !== 0 ?
                                    <Select value={surgerySubHistory} onChange={(value) => handleChangesub_surgeryHistory(value)} placeholder="SURGERY-HISTORY-SUB-TYPE" className="Surgery-type-dropdown" id="patien-type" options={sub_surgeryHistoryArray} />
                                    : <></>
                            }
                        ...
                        <div className="medication-extra-type-container" >
                            <input className="medication-input" onChange={(e) => setcategoryValue(e.target.value)} placeholder="enter not listed type" />
                            <div className="add-more-button" onClick={() => addotherIntoSurgeryHistoryCategory(categoryValue)} >Add</div>
                        </div>
                        {
                                    ...
                                    return (
                                        ...
                                        <div className="sub-question-container" >
                                            ...
                                            {anesethesiaSubHistory.length === 0 ?
                                                <div className="option-box-container" >
                                                    <input onChange={(e) => onAnesthesiaHistoryChanged(e, data1)} type="radio" value="Yes" name={data1.name} /> Yes

                                                    <input onChange={(e) => onAnesthesiaHistoryChanged(e, data1)} type="radio" value="No" name={data1.name} /> No
                                                </div> : <></>}
                                            {data1.label === "HISTORY OF COMPLICATIONS WITH ANESTHESIA" || data1.label === "FAMILY HISTORY OF COMPLICATIONS WITH ANESTHESIA" ?
                                                <div>
                ...
                {questinNo == 13 ?
                    <div className="question-container" >
                        ...
                        <div className="ANESTHESIA-type-input-container" >
                            <Select value={allegries} isMulti={true} onChange={(value) => handleChangeAllegries(value)} placeholder="ALLERGIES" className="ANESTHESIA-type-dropdown" id="patien-type" options={AllegriesArray} />
                            <div className="medication-extra-type-container" >
                                <input className="medication-input" onChange={(e) => setcategoryValue(e.target.value)} placeholder="enter not listed type" />
                                <div className="add-more-button" onClick={() => addotherIntoAllgriesCategory(categoryValue)} >Add</div>
                            </div>
                            {
                                otherAllegriesArray.map((data) => {
                                    return (
                ...
                {questinNo == 14 ?
                    <div className="question-container" >
                        ...
                        <div className="age-input-container" >
                            <input type="number" value={timeOfLastFoodOrDrink} onChange={(e) => settimeOfLastFoodOrDrink(e.target.value)} placeholder="TIME OF LAST FOOD OR DRINK" className="age-input" />
                            {optionForNPOStatus.map((data) => {
                                if (data.value === npo_status_time_type) {
                                    return (
                                        <div className="npo-time-type-selected-type-button" onClick={() => handleChangeNPOStatus(data.value)} >{data.label}</div>
                                    )
                                } else {
                                    return (
                                        <div className="weight-type-button" onClick={() => handleChangeNPOStatus(data.value)} >{data.label}</div>
                        ...
                        <div className="ingested-input-container" >
                            {ingestedMaterialArray.map((data) => {
                                if (data === ingestedMaterial) {
                                    return (
                                        <div className="ingested-selected-type-button" onClick={() => saveIngestedMaterial(data)}  >{data.label}</div>
                                    )
                                } else {
                                    return (
                                        <div className="ingested-type-button" onClick={() => saveIngestedMaterial(data)}  >{data.label}</div>
